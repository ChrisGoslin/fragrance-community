...
const STAMPS: { value: 'liked' | 'disliked' | 'unworn'; label: string; emoji: string; activeClass: string }[] = [
  { value: 'liked',    label: 'Like',         emoji: '❤️', activeClass: 'bg-rose-50 border-rose-400 text-rose-500' },
  { value: 'disliked', label: 'Dislike',       emoji: '👎', activeClass: 'bg-gray-100 border-gray-400 text-gray-500' },
  { value: 'unworn',   label: "Haven't worn",  emoji: '🤍', activeClass: 'bg-blue-50 border-blue-300 text-blue-400' },
]
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {fragrances.map(f => {
        ...
        return (
          ...
          >
            <div>
              <p className="text-xs font-medium uppercase tracking-widest text-gray-400">{f.brand}</p>
              <h2 className="text-sm font-semibold mt-1 leading-snug text-gray-900">{f.name}</h2>
            </div>

            <div className="flex flex-wrap gap-1">
              {f.concentration && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{f.concentration}</span>
              )}
              {f.gender_profile && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{f.gender_profile}</span>
              )}
              {f.layering_role && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-purple-50 text-purple-500">{f.layering_role}</span>
              )}
            </div>
