...
function vectorColor(vector: string): string {
  ...
  if (v.includes("aquatic") || v.includes("marine") || v.includes("fresh"))
    return "#e0f2fe";
...
export default function LibraryPage() {
  ...
  async function addToCollection(
    ...
    if (error) {
      ...
      showToast(
        `${fragrance.name} added to ${status === "owned" ? "your shelf" : "wishlist"}`
      );
        ...
        <div>
          ...
          {search.trim() === "" ? (
            <div style={styles.emptyState}>
              <div style={styles.emptyIcon}>🔍</div>
              <p style={styles.emptyText}>
                Search across {fragrances.length} fragrances by brand, name, scent family, or dupe reference.
              </p>
            </div>
                    ...
                    <div style={styles.pyramid}>
                      {f.top_notes?.length > 0 && (
                        <div style={styles.pyramidRow}>
                          ...
                          <span style={styles.pyramidNotes}>
                            {f.top_notes.join(", ")}
                          </span>
                        </div>
                      )}
          ...
          <div style={styles.controlGrid}>
            ...
            <div style={styles.controlGroup}>
              ...
              <select
                ...
                onChange={(e) =>
                  onUpdate(item.id, { wear_state: e.target.value })
                }
              >
            ...
            <div style={styles.controlGroup}>
              ...
              <select
                ...
                onChange={(e) =>
                  onUpdate(item.id, { shelf_tier: Number(e.target.value) })
                }
              >
                {[1, 2, 3, 4].map((t) => (
                  <option key={t} value={t}>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  cardBrand: { fontSize: 12, color: "#6b7280", fontWeight: 500, textTransform: "uppercase", letterSpacing: 0.5 },
  ...
  controlLabel: { fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.4 },
  textarea: {
    padding: "8px 10px",
    border: "1px solid #e5e7eb",
    borderRadius: 8,
  ...
  searchBrand: { fontSize: 11, color: "#6b7280", fontWeight: 500, textTransform: "uppercase", letterSpacing: 0.5 },
